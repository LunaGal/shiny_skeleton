...
relabun_by_group <- function(table_input, group = "Marker", taxlevel = "species") {
  ...
  # Count the total number of each marker
  ...
  # Filter out rows with blanks in the "species" column
...
raw_table <- read.csv("Van_eDNAFlow/Fishes.df4.csv")
test_table <- relabun_by_group(raw_table)
getPalette <- colorRampPalette(brewer.pal(12, "Paired"))
colourCount <- length(unique(test_table$species))
